...
def wvStd(a):
	return [np.std(x[0], x[1]) for x in a]
	...
	def __init__(self, data=None, featurenames=[]):
		...
		if data is not None:
			...
			for pair in data.train():
				...
				self.train.append(featurize(pair, features))
			for pair in data.test():
				self.testlabels.append(pair.label)
				self.test.append(featurize(pair, features))
...
def processData(args):
	...
	features = {x for x in args.basefeatures.split(',')}
	matchers = {x for x in args.matchers.split(',')}
	...
	if "MinDistSim" in matchers:
		...
		comparator = 'cosine'
		matcher = vectorsim.MinDistSim(metric=comparator, maxsent=maxwords, dimfeatures=args.dimfeatures)
	...
	if "NGram" in matchers:
		...
		comparator = 'cosine'
		matcher = vectorsim.MinDistSim(metric=comparator, maxsent=maxwords, ngram=2, dimfeatures=args.dimfeatures)
		fs.addMatcher(matcher, 'cos-bigram')
		comparator = 'cosine'
		matcher = vectorsim.MinDistSim(metric=comparator, maxsent=maxwords, ngram=3, dimfeatures=args.dimfeatures)
...
def main(args):
	...
	evalData(model=model, data=fs.train, labels=lintrainlabels, classify=args.classify, **predictargs)
	# trainobs = _
	...
	if args.writematches:
		writer = fs.data.writer(args.writematches)
		for pair in fs.data.train():
			writer.write(pair, Match(score=pair.label, autop=0))
		for pair, obs in izip(fs.data.test(), testobs):
			writer.write(pair, Match(score=obs))
...
def cmdline():
	...
	defaults = {
		...
		"matchers": "InfSim,InfRankSim,MinDistSim,WWSim,Pair",
		#"min_samples_split": 2,
		"model": "randomforests",
		"neg_samples": 5,
		#"n_estimators": 40,
	...
	argparser.add_argument('--model', help='Type of model to use (randomforests, gradientboosting, decisiontree, extratrees, autolearn)')
